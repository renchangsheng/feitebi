...
public class UserAccountDao {
	...
	public StatusBean sendCode(String userid,String phone){	
		...
		try { 
			...
			if (SendSMSValidCode.sendSMS(phone,"【币大师】短信验证码是:" + four)) {
	...
	public StatusBean updatePhone(String userid,String phone){	
		StatusBean status = new StatusBean();
		status.setFlag(0);
	  	Connection conn = null;
    	PreparedStatement stmt= null;
		try {
			conn = DBUtil.getInstance().getConnection();
    		StringBuilder buffer=new StringBuilder();
 	        buffer.append("update user set phone=? where userId=?");
 	        stmt=conn.prepareStatement(buffer.toString());
